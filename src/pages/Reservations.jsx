...
const Reservations = () => {
    ...
    const handleNext = (index) => {
        setCurrentPhotoIndex((prev) => ({
            ...prev,
            [index]: (prev[index] ?? 0) + 1 < userTrips[index].photos.length ? (prev[index] ?? 0) + 1 : 0,
        }));
    };
    ...
    const handlePrev = (index) => {
        setCurrentPhotoIndex((prev) => ({
            ...prev,
            [index]: (prev[index] ?? 0) - 1 >= 0 ? (prev[index] ?? 0) - 1 : userTrips[index].photos.length - 1,
        }));
    };
        ...
        <div className="bg-gray-100 min-h-screen p-6 w-screen">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 cursor-pointer">
                {userTrips.map((trip, index) => (
                    <div key={trip.booking_id} className="bg-white rounded-lg shadow-lg overflow-hidden">
                        <div className="relative w-full h-64">
                            <img onClick={() => handleClick(trip.listing_id)}
                        ...
                        <div className="p-4">
                            ...
                            <p className="text-gray-600">Stay from: {new Date(trip.start_date).toLocaleDateString()}</p>
                            <p className="text-gray-600">Stay until: {new Date(trip.end_date).toLocaleDateString()}</p>
                            <p className="text-lg font-bold text-blue-600">Total Price: ${trip.total_price}</p>
                        </div>
                    </div>
                ))}
            </div>
